...
from receives.call import Call
...
class Patch():
    ...
    def patch(self):
        mock = MagicMock(side_effect=self._handle_call)
        self._original_call = getattr(self.context.object, self.context.attribute_name)
        setattr(self.context.object, self.context.attribute_name, mock)

    def unpatch(self):
        if self.original_call is not None:
            setattr(self.context.object, self.context.attribute_name, self.original_call)

    def finalize(self):
        self.unpatch()

    def _handle_call(self, *kargs, **kwargs):
        call = Call(self.context, kargs, kwargs)
        expectation = self.mapping.find_expectation_for(call)

        if expectation is None:
            raise AssertionError("No expectation found")

        result = expectation.validate(kargs, kwargs)
        return result
